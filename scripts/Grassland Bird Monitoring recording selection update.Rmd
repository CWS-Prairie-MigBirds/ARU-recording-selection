...
```{r}
dir.create("D:/GBMP/2025/WildTrax Upload") #only needs to be run once to create the folder

new.folder.10M1SPT <- "D:/GBMP/2025/WildTrax Upload/10M1SPT"
new.folder.3M1SPT <- "D:/GBMP/2025/WildTrax Upload/3M1SPT"
new.folder.dusk <- "D:/GBMP/2025/WildTrax Upload/DUSK"
...
data <- data  %>% mutate(category= NA) %>% 
  mutate(start.date=min(JDay),end.date= median(JDay),start.time=as_hms(sunrise-(35*60)),end.time=as_hms(sunrise +(2.1*60*60))) %>% 
  categorize("EE",start.date,end.date,start.time,end.time) %>% 
  
  mutate(start.date=min(JDay),end.date= median(JDay),start.time= as_hms(sunrise+(2.2*60*60)),end.time= as_hms(sunrise+(4.1*60*60)))%>% 
  categorize("EL",start.date,end.date,start.time,end.time) %>%
  
  mutate(start.date=median(JDay) +1, end.date= max(JDay),start.time=as_hms(sunrise -(35*60)),end.time=as_hms(sunrise +(2.1*60*60))) %>% 
  categorize("LE",start.date,end.date,start.time,end.time) %>% 

  mutate(start.date= median(JDay) + 1, end.date= max(JDay),start.time= as_hms(sunrise+(2.2*60*60)),end.time= as_hms(sunrise+(4.1*60*60)))%>% 
