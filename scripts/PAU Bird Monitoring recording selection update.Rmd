---
title: "Protected Areas Bird Monitoring Program - Recording Selection Workflow"
author: Janine McManus, Jeff Harder, Barry Robinson, Judith Toms, Megan Edgar, Steve Van Wilgenburg
output: html_document
date: "2025-08-26"
---

The following script will select the dates and times based on good weather at one station, and then copy recordings with the same dates and times for all other stations in a given NWA that were deployed for the same period of time.

This script will only work when recordings are stored with each NWA-deployment as a directory, and each ARU station within an NWA-deployment as a separate sub-directory. 
...
Enter the parent directory that contains all the recordings for all NWAs in the given year for which you want to run the selection.

```{r}
parentdir <- "D:/PAU/2025"
```
...
```{r}
dir.create("D:/PAU/2025/WildTrax Upload") #only needs to be run once to create the folder

new.folder.10M1SPT <- "D:/PAU/2025/WildTrax Upload/10M1SPT"
new.folder.3M1SPT <- "D:/PAU/2025/WildTrax Upload/3M1SPT"
new.folder.dusk <- "D:/PAU/2025/WildTrax Upload/DUSK"
...
dir.create(new.folder.night) #only needs to be run once to create the folder
```
...
#Step 1. Set your NWA-deployment to run for selection by replacing the name of the NWA in the quotations below to match how you have named your folder for that site.

**Note, if there were two rounds of deployments at a given NWA, keep the ARU stations for each deployment in separate folders, for example Sandhills_1 and Sandhills_2
...
Note that if the first station folder is not very good for some reason (e.g., ARU failure), you can set the code below to use the second or alternate folder by changing folder number at end of line 89.

```{r, setup}

knitr::opts_knit$set(root.dir = list.dirs(paste(parentdir, NWA_deployment, sep = "/"), recursive = F)[1])

```

# Step 3. Get .wav data for one ARU station from the NWA
...
PAU in prairie region need two 5-minute recordings from right around sunrise, 1 10-minute to be transcribed by terrestrial unit and three 3-minute dusk recordings (PAU to transcribe just 2 to start)

1 morning recording should be selected within the first half of the deployment (Early_Deployment), and two recordings will be selected from the second half (Late_Deployment)

Night recordings are selected randomly across the entire deployment

I've added a little wiggle room in the minutes to account for small variations in ARU programming.  

NE = Night Early; NM = Night Middle; NL = Night Late
...
data <- data  %>% mutate(category= NA) %>% 
  ...
  mutate(start.date=min(JDay),end.date= median(JDay),start.time=as_hms(sunrise-(35*60)),end.time=as_hms(sunrise +(135*60))) %>% 
  categorize("AMEarly_Deployment",start.date,end.date,start.time,end.time) %>% 
  
  mutate(start.date=median(JDay)+1,end.date= max(JDay),start.time=as_hms(sunrise - (35*60)),end.time=as_hms(sunrise +(135*60))) %>% 
  categorize("AMLate_Deployment",start.date,end.date,start.time,end.time) %>% 
 
  mutate(start.date=min(JDay),end.date= max(JDay),start.time= as_hms(sunset-(35*60)),end.time= as_hms(sunset+(35*60)))%>%
  categorize("NE",start.date,end.date,start.time,end.time) %>%
  
  mutate(start.date=min(JDay),end.date= max(JDay),start.time= as_hms(sunset+(55*60)),end.time= as_hms(sunset+(3600*4)))%>%
  categorize("NM",start.date,end.date,start.time,end.time) %>%
  
  mutate(start.date=min(JDay),end.date= max(JDay),start.time=as_hms(sunset+(3600*4)),end.time=as_hms(sunset+(3600*7))) %>%
  categorize("NL",start.date,end.date,start.time,end.time) %>%
  
  dplyr::select(-start.date, -end.date, -start.time, -end.time) 
...
```{r}
night <- subset(data, category %in% c("NE", "NM", "NL"))
morn <- subset(data, category %in% c("AMEarly_Deployment", "AMLate_Deployment"))
```
...
```{r}
MorningDraw <- morn %>%  group_by(category) %>% sample_n(6)


NightDraw <- night  %>% sample_n(12)
...
knitr::opts_knit$set(root.dir = paste(parentdir, NWA_deployment, sep = "/"))
...
Early deployment AM
```{r}
file_used_early <- select_recordings(MorningDraw, "Morning", initial_file_index = 1, target_folder = new.folder.10M1SPT)
```

Late deployment AM
```{r}
file_used_late <- select_recordings(MorningDraw, "Morning", initial_file_index = 7, target_folder = new.folder.3M1SPT)
```

Third visit AM
```{r}
file_used_third <- select_recordings(MorningDraw, "Morning", initial_file_index = 13, target_folder = new.folder.3M1SPT)
```
...
Now return to Step 1 and change the NWA deployment name to the next one you need to run selection for and work through the script again!
